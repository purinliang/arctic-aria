...
Ideas persistence, Discord capture, and basic web management are implemented.
Discord `/idea` can create untriaged ideas for a bound Arctic Aria user. The
web Ideas page can create, edit, list, and delete ideas. User-facing triage and
conversion controls are not implemented yet.
...
- web add, edit, and delete management for raw ideas
...
The first capture and edit commands store one raw text value. The user should
not need to choose a title, tags, destination feature, deadline, or priority
during capture.

Backend validation should trim the text, reject empty text, and reject text
longer than 2,000 characters. This keeps Discord capture, web create, web edit,
and database storage predictable.
