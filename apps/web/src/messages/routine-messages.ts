import type { RoutineRuleType } from "@/features/dashboard/types";
import type { RoutineRecurrenceOption } from "@/features/routines/routine-recurrence";

export const englishRoutineMessages = {
  page: {
    title: "Routines",
    description: "Routines due today.",
    new: "New",
    loading: "Loading routines...",
    empty: "No routines yet.",
    noDescription: "No description.",
    flexible: "Flexible",
    edit: "Edit",
  },
  groups: {
    title: "Groups",
    description: "Filter routines by life area.",
    all: "All",
    noGroup: "Default",
    manage: "Manage",
    manageTitle: "Manage Groups",
    closeEditor: "Close group manager",
    closeForm: "Close group form",
    sectionTitle: "Routine Groups",
    new: "New",
    edit: "Edit",
    add: "Add group",
    editTitle: "Edit group",
    name: "Group name",
    namePlaceholder: "Group name",
    descriptionLabel: "Description",
    noDescription: "No description.",
    noGroups: "No groups yet.",
    save: "Save",
    saving: "Saving",
    delete: "Delete",
    confirmTitle: "Delete group",
    confirmFallback: "this group",
    confirmDescription: (name: string) =>
      `Delete "${name}"? Routines in this group will move to Default.`,
    defaults: [
      "Routines that belong to the same part of life.",
      "A place for related recurring checks.",
      "A simple group for routines you review together.",
    ],
  },
  instances: {
    title: "Routine Instances",
    description: "Generated routine actions.",
    loading: "Loading routine instances...",
    empty: "No routine instances in this range.",
    complete: "Complete",
    reopen: "Reopen",
    markDone: (title: string) => `Mark "${title}" done`,
    reopenItem: (title: string) => `Reopen "${title}"`,
    status: {
      pending: "Pending",
      completed: "Completed",
      skipped: "Skipped",
    },
  },
  filters: {
    title: "Filter",
    description: "Choose which instances to show.",
    all: "All",
    recent: "Recent",
    future: "Future",
    past: "Past",
  },
  editor: {
    add: "Add routine",
    edit: "Edit routine",
    close: "Close routine editor",
    title: "Title",
    titlePlaceholder: "Routine title",
    description: "Description",
    group: "Group",
    recurrence: "Repeat rule",
    estimatedDuration: "Estimated duration",
    estimatedDurationPlaceholder: "Minutes",
    fixedIntervalDays: "Days between repeats",
    startDate: "Start date",
    endDate: "End date",
    preferredTime: "Preferred time",
    timezone: "Timezone",
    timezoneHint:
      "Check this for routines tied to overseas meetings or another local clock.",
    selectFirstDate: "Start date",
    selectEndDate: "End date",
    selectTime: "Select time",
    preview: "Next",
    previewUnavailable: "Choose a start date to preview repeats.",
    andSoOn: "and so on...",
    save: "Save",
    saving: "Saving",
    delete: "Delete",
    template: {
      menuLabel: "Template",
      menuAriaLabel: "Routine editor actions",
      title: "Routine Template",
      close: "Close routine template",
      editTab: "Template",
      previewTab: "Preview",
      copyTemplate: "Copy",
      copied: "Template copied.",
      copyFailed: "Template could not be copied.",
      inputPlaceholder: "Paste the filled routine template here.",
      parse: "Preview",
      parsing: "Previewing...",
      apply: "Save",
      applying: "Saving...",
      previewEmpty: "Parse the template to preview changes.",
      previewCounts: (
        create: number,
        update: number,
        deleteCount: number,
        preserve: number,
      ) =>
        `${create} create · ${update} update · ${deleteCount} delete · ${preserve} preserve`,
      ignoredFieldsTitle: "Template parsed with warnings",
      ignoredFields: (count: number) =>
        count === 1
          ? "1 unsupported template field was ignored."
          : `${count} unsupported template fields were ignored.`,
      subject: "Routine",
      operationBadges: {
        create: "Create",
        update: "Update",
        delete: "Delete",
        preserve: "Preserve",
      },
    },
  },
  recurrenceOptions: {
    once: "Once",
    daily: "Daily",
    weekly: "Weekly",
    monthly: "Monthly",
    yearly: "Yearly",
    every_14_days: "Every 14 days",
    every_30_days: "Every 30 days",
    fixed_days: "Fixed interval",
  } satisfies Record<RoutineRecurrenceOption, string>,
  recurrenceDescriptions: {
    once: "For a one-time check on the start date.",
    daily: "For habits or checks that matter every day.",
    weekly: "Every 7 days from the start date.",
    monthly: "For bills and renewal checks on a calendar day.",
    yearly: "For annual checks on the same calendar date.",
    every_14_days: "Every 14 days from the start date.",
    every_30_days: "A fixed 30-day cycle, not a calendar month.",
    fixed_days: "For a custom cycle such as every 70 or 300 days.",
  } satisfies Record<RoutineRecurrenceOption, string>,
  rules: {
    once: "Once",
    daily: "Daily",
    weekly: "Weekly",
    bi_weekly: "Every two weeks",
    monthly_by_date: "Monthly",
    day_interval: "Every few days",
  } satisfies Record<RoutineRuleType, string>,
  ruleDescriptions: {
    once: "Show this routine once on the start date.",
    daily: "Show this routine every day.",
    weekly: "Repeat every 7 days from the start date.",
    bi_weekly: "Repeat every 14 days from the start date.",
    monthly_by_date: "Repeat monthly from the start date.",
    day_interval: "Repeat after a custom number of days.",
  } satisfies Record<RoutineRuleType, string>,
  weekdays: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"],
  summary: {
    noDaySelected: "No day selected",
    once: "Once",
    weekly: (days: string) => `Weekly: ${days}`,
    every14Days: "Every 14 days",
    monthly: (interval: number, day: number) =>
      interval === 12
        ? `Yearly on this date`
        : interval === 1
        ? `Monthly on day ${day}`
        : `Every ${interval} months on day ${day}`,
    dayInterval: (interval: number) => `Every ${interval} days`,
  },
  defaultDescriptions: [
    "A repeatable check to keep life moving.",
    "A routine to return to when it is due.",
    "A recurring item worth checking off.",
    "A small repeated action to stay on track.",
    "A regular reminder for everyday maintenance.",
  ],
  confirm: {
    title: "Delete routine",
    fallback: "this routine",
    description: (title: string) =>
      `Delete "${title}"? It will be removed from normal views.`,
    cancel: "Cancel",
    confirm: "Delete",
    deleting: "Deleting",
    close: "Close confirmation",
  },
  results: {
    auth_required: "Please sign in again.",
    routine_title_invalid: "Routine title must be 1-120 characters.",
    routine_description_invalid:
      "Routine description must be 2000 characters or fewer.",
    routine_estimated_duration_invalid:
      "Estimated duration must be 1-1440 minutes.",
    routine_start_date_missing: "Choose a start date.",
    routine_start_date_invalid:
      "Start date must be a real date in YYYY-MM-DD format.",
    routine_end_date_invalid:
      "End date must be blank or after the start date.",
    routine_preferred_time_invalid: "Preferred time must use HH:MM.",
    routine_timezone_invalid: "Choose a valid timezone.",
    routine_rule_invalid: "Routine rule is invalid.",
    routine_weekday_missing: "Choose at least one weekday.",
    routine_not_found: "Routine was not found.",
    routine_instance_not_found: "Routine instance was not found.",
    routine_group_description_invalid:
      "Routine group description must be 500 characters or fewer.",
    routine_group_duplicate: "A routine group with that name already exists.",
    routine_group_invalid: "Choose a valid routine group.",
    routine_group_name_invalid: "Routine group name must be 1-80 characters.",
    routine_group_not_found: "Routine group was not found.",
    routine_template_invalid: "Routine template is invalid.",
    routine_template_missing: "Paste a routine template first.",
    routine_template_target_mismatch:
      "Template routine does not match this routine.",
  },
};

export type RoutineMessages = typeof englishRoutineMessages;

export const simplifiedChineseRoutineMessages: RoutineMessages = {
  page: {
    title: "日常",
    description: "今天需要处理的日常。",
    new: "新建",
    loading: "正在加载日常...",
    empty: "还没有日常。",
    noDescription: "暂无描述。",
    flexible: "灵活时间",
    edit: "编辑",
  },
  groups: {
    title: "分组",
    description: "按生活领域筛选日常。",
    all: "全部",
    noGroup: "默认",
    manage: "管理",
    manageTitle: "管理分组",
    closeEditor: "关闭分组管理",
    closeForm: "关闭分组表单",
    sectionTitle: "日常分组",
    new: "新建",
    edit: "编辑",
    add: "添加分组",
    editTitle: "编辑分组",
    name: "分组名称",
    namePlaceholder: "分组名称",
    descriptionLabel: "描述",
    noDescription: "暂无描述。",
    noGroups: "还没有分组。",
    save: "保存",
    saving: "正在保存",
    delete: "删除",
    confirmTitle: "删除分组",
    confirmFallback: "这个分组",
    confirmDescription: (name) =>
      `删除“${name}”？这个分组里的日常会移动到默认分组。`,
    defaults: [
      "把同一类日常放在一起。",
      "适合整理相关重复事项。",
      "方便一起查看的一组日常。",
    ],
  },
  instances: {
    title: "日常实例",
    description: "已生成的具体日常行动。",
    loading: "正在加载日常实例...",
    empty: "这个范围内没有日常实例。",
    complete: "完成",
    reopen: "重新打开",
    markDone: (title) => `完成“${title}”`,
    reopenItem: (title) => `重新打开“${title}”`,
    status: {
      pending: "待处理",
      completed: "已完成",
      skipped: "已跳过",
    },
  },
  filters: {
    title: "筛选",
    description: "选择要显示的日常实例。",
    all: "全部",
    recent: "近期",
    future: "未来",
    past: "过去",
  },
  editor: {
    add: "添加日常",
    edit: "编辑日常",
    close: "关闭日常编辑器",
    title: "标题",
    titlePlaceholder: "日常标题",
    description: "描述",
    group: "分组",
    recurrence: "重复规则",
    estimatedDuration: "预计用时",
    estimatedDurationPlaceholder: "分钟",
    fixedIntervalDays: "重复间隔天数",
    startDate: "开始日期",
    endDate: "结束日期",
    preferredTime: "偏好时间",
    timezone: "时区",
    timezoneHint: "适合海外会议，或需要按另一个本地时间提醒的日常。",
    selectFirstDate: "开始日期",
    selectEndDate: "结束日期",
    selectTime: "选择时间",
    preview: "下次",
    previewUnavailable: "选择开始日期后可预览重复日期。",
    andSoOn: "以此类推",
    save: "保存",
    saving: "正在保存",
    delete: "删除",
    template: {
      menuLabel: "模板",
      menuAriaLabel: "日常编辑操作",
      title: "日常模板",
      close: "关闭日常模板",
      editTab: "模板",
      previewTab: "预览",
      copyTemplate: "复制",
      copied: "模板已复制。",
      copyFailed: "模板无法复制。",
      inputPlaceholder: "在这里粘贴填写后的日常模板。",
      parse: "预览",
      parsing: "正在预览...",
      apply: "保存",
      applying: "正在保存...",
      previewEmpty: "先解析模板以预览更改。",
      previewCounts: (create, update, deleteCount, preserve) =>
        `${create} 个创建 · ${update} 个更新 · ${deleteCount} 个删除 · ${preserve} 个保留`,
      ignoredFieldsTitle: "模板解析有提醒",
      ignoredFields: (count) => `${count} 个不支持的模板字段已被忽略。`,
      subject: "日常",
      operationBadges: {
        create: "创建",
        update: "更新",
        delete: "删除",
        preserve: "保留",
      },
    },
  },
  recurrenceOptions: {
    once: "一次",
    daily: "每天",
    weekly: "每周",
    monthly: "每月",
    yearly: "每年",
    every_14_days: "每 14 天",
    every_30_days: "每 30 天",
    fixed_days: "固定间隔",
  },
  recurrenceDescriptions: {
    once: "只在开始日期显示一次。",
    daily: "适合每天都需要检查的习惯或事项。",
    weekly: "从开始日期起，每 7 天重复。",
    monthly: "适合账单和续费检查，按日历日期重复。",
    yearly: "适合每年同一天检查的事项。",
    every_14_days: "从开始日期起，每 14 天重复。",
    every_30_days: "固定 30 天周期，不是日历月。",
    fixed_days: "适合自定义周期，例如每 70 天或 300 天。",
  },
  rules: {
    once: "一次",
    daily: "每天",
    weekly: "每周",
    bi_weekly: "每两周",
    monthly_by_date: "每月",
    day_interval: "每隔几天",
  },
  ruleDescriptions: {
    once: "只在开始日期显示这个日常。",
    daily: "每天显示这个日常。",
    weekly: "从开始日期起，每 7 天重复。",
    bi_weekly: "从开始日期起，每 14 天重复一次。",
    monthly_by_date: "从开始日期起，每月重复。",
    day_interval: "按自定义天数间隔重复。",
  },
  weekdays: ["日", "一", "二", "三", "四", "五", "六"],
  summary: {
    noDaySelected: "未选择日期",
    once: "一次",
    weekly: (days) => `每周：${days}`,
    every14Days: "每 14 天",
    monthly: (interval, day) =>
      interval === 12
        ? "每年这一天"
        : interval === 1
          ? `每月第 ${day} 天`
          : `每 ${interval} 个月的第 ${day} 天`,
    dayInterval: (interval) => `每 ${interval} 天`,
  },
  defaultDescriptions: [
    "帮助生活继续运转的重复检查。",
    "到时间就回来看看的日常。",
    "值得定期完成的事项。",
    "让节奏保持稳定的小动作。",
    "适合日常维护的定期提醒。",
  ],
  confirm: {
    title: "删除日常",
    fallback: "这个日常",
    description: (title) => `删除“${title}”？它会从常规视图中移除。`,
    cancel: "取消",
    confirm: "删除",
    deleting: "正在删除",
    close: "关闭确认对话框",
  },
  results: {
    auth_required: "请重新登录。",
    routine_title_invalid: "日常标题必须为 1-120 个字符。",
    routine_description_invalid: "日常描述必须不超过 2000 个字符。",
    routine_estimated_duration_invalid: "预计用时必须为 1-1440 分钟。",
    routine_start_date_missing: "请选择开始日期。",
    routine_start_date_invalid: "开始日期必须是真实日期，格式为 YYYY-MM-DD。",
    routine_end_date_invalid: "结束日期必须为空，或晚于开始日期。",
    routine_preferred_time_invalid: "偏好时间必须使用 HH:MM 格式。",
    routine_timezone_invalid: "请选择有效时区。",
    routine_rule_invalid: "日常规则无效。",
    routine_weekday_missing: "请至少选择一个星期。",
    routine_not_found: "没有找到日常。",
    routine_instance_not_found: "没有找到日常实例。",
    routine_group_description_invalid: "日常分组描述必须不超过 500 个字符。",
    routine_group_duplicate: "同名日常分组已存在。",
    routine_group_invalid: "请选择有效的日常分组。",
    routine_group_name_invalid: "日常分组名称必须为 1-80 个字符。",
    routine_group_not_found: "没有找到日常分组。",
    routine_template_invalid: "日常模板无效。",
    routine_template_missing: "请先粘贴日常模板。",
    routine_template_target_mismatch: "模板中的日常与当前日常不一致。",
  },
};
