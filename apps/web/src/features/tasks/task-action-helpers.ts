...
export function unauthorizedResult<T>(): TaskActionResult<T> {
  return {
    ok: false,
...
export async function loadTaskDashboardData(
  userId: string,
...
export function validateTaskInput(input: TaskInput) {
  ...
  if (planTitle && planTitle.length > 120) {
    return {
      ok: false as const,
      message: "Plan name must be 120 characters or fewer.",
    };
  }
...
export function validateProgressInput(input: TaskProgressInput) {
  const weight = Number(input.weight);
  const completedWeight = Number(input.completedWeight);
...
function validateChild(
  child: ReturnType<typeof normalizeChildren>[number],
...
function normalizeChildren(input: TaskInput) {
