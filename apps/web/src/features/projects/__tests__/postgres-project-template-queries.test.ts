...
import { createProjectTreeTemplate } from "../server/postgres-project-template-create-queries.ts";
...
test("project tree template apply uses one guarded SQL statement", async () => {
  ...
  assert.match(record.text, /milestone_task_delete AS/);
...
test("project tree template create uses one guarded SQL statement", async () => {
  const { records, sql } = createSqlStub([{ id: "project-1" }]);
  const input: CreateProjectTreeTemplateInput = {
    userId: "user-1",
    project: {
    ...
    milestones: [
      {
        milestoneId: "00000000-0000-0000-0000-000000000011",
        title: "Foundation",
        objective: null,
        startDate: "2026-07-29",
        deadlineDate: null,
  ...
  assert.equal(await createProjectTreeTemplate(sql, input), "project-1");
  ...
  assert.equal(records.length, 1);
  assert.match(record.text, /WITH milestone_input AS/);
  assert.match(record.text, /project_create AS/);
  assert.match(record.text, /milestone_create AS/);
  assert.match(record.text, /task_create AS/);
  assert.match(record.text, /SELECT id\s+FROM project_create/);
  assert.equal(record.params.length, 10);
  assert.deepEqual(JSON.parse(String(record.params[8])), [
    {
      milestone_id: "00000000-0000-0000-0000-000000000011",
      title: "Foundation",
      objective: null,
      start_date: "2026-07-29",
      deadline_date: null,
...
function createSqlStub(resultRows: unknown[] = [{ ok: true }]) {
